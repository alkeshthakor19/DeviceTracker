...
fun HomeScreen(openDrawer: () -> Unit) {
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        Surface(modifier = Modifier.weight(1f)) {
            ...
            val tabCount = list.filter { it.type == DeviceType.TAB.ordinal }.size
            val usbCount = list.filter { it.type == DeviceType.USB.ordinal }.size
            val probeCount = list.filter { it.type == DeviceType.PROB.ordinal }.size
